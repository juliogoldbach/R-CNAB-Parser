...
ajustar_cnab_file <- function() {
  ...
  col.names <- c("Identificação do Registro", "Agência de Débito (opcional)", "Dígito da Agência de Débito (opcional)", "Razão da Conta Corrente (opcional)", "Conta Corrente (opcional)",
                 "Dígito da Conta Corrente (opcional)", "Identificação da Empresa Beneficiária no Banco", "Num. Controle do Participante", "Código do Banco a ser debitado na Câmara de Compensação",
